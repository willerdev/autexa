...
/** Mirrors server inferFallbackWidgets — used if API omits `widgets` (older server) or model skips the tool. */
export function inferLocalChatWidgets(answer: string, userMessage = ''): ChatWidgetSpec[] {
  const text = `${answer}\n${userMessage}`;
  const out: ChatWidgetSpec[] = [];
  ...
  const needPhoto = /\b(photo|picture|image|camera|snap|take a pic|send (me )?(a )?photo|attach (a )?photo|show me)\b/i.test(
    answer,
  );
  ...
  if (needDate) {
    out.push({ type: 'date_picker', label: 'Choose date', hint: 'Tap to open the calendar' });
  }
  if (needTime) {
    out.push({ type: 'time_picker', label: 'Choose time', hint: 'Tap to set the clock' });
  }
  if (needPhoto) {
    out.push({ type: 'photo_capture', label: 'Photo', hint: 'Camera or photo library' });
  }
  if (needAudio) {
    out.push({ type: 'audio_record', label: 'Record sound', hint: 'Hold the phone near the noise', max_seconds: 45 });
  }
  if (needPayment) {
    out.push({
